...
//       // Fetch related listings after we have the current listing data
//       if (data) {
//         fetchRelatedListings(data.category, data.user_id, data.id);
//       }
//     } catch (error: any) {
...
//   const fetchRelatedListings = async (category: string, sellerId: string, currentListingId: string) => {
//     setLoadingRelated(true);
//     try {
//       // Fetch listings from same category (excluding current listing)
...
//       // Fetch other listings from same seller (excluding current listing)
//       const { data: sellerOtherListings, error: sellerError } = await supabase
//         .from('listings')
//         .select('*')
//         .eq('user_id', sellerId)
//         .eq('status', 'active')
...
//       const allRelated = [...(categoryListings || []), ...(sellerOtherListings || [])];
//       const uniqueRelated = allRelated.filter((listing, index, self) => 
//         index === self.findIndex(l => l.id === listing.id)
//       );

//       // Separate related listings and seller listings
//       const relatedFromOthers = uniqueRelated.filter(listing => listing.user_id !== sellerId);
//       const fromSameSeller = sellerOtherListings || [];

//       setRelatedListings(relatedFromOthers.slice(0, 4)); // Show max 4 related listings
//       setSellerListings(fromSameSeller.slice(0, 4)); // Show max 4 seller listings
//     } catch (error: any) {
...
//   const handleRelatedListingPress = (listingId: string) => {
//     router.push(`/listing/${listingId}`);
//   };

//   const renderRelatedListing = ({ item }: { item: Listing }) => (
//     <TouchableOpacity 
//       style={styles.relatedCard}
//       onPress={() => handleRelatedListingPress(item.id)}
//       activeOpacity={0.7}
//     >
//       <Image
...
//               {renderSectionHeader(`More from ${listing.user?.name || 'this seller'}`, sellerListings.length)}
//               <FlatList
//                 data={sellerListings}
...
//           {/* Related Products from Other Sellers */}
//           {relatedListings.length > 0 && (
//             <View style={styles.relatedSection}>
//               {renderSectionHeader('Similar Products', relatedListings.length)}
//               <FlatList
...
//       {/* Loading indicator for related listings */}
//       {loadingRelated && (
...
//     marginBottom: 12,
//   },
//   sectionHeaderTitle: { // Renamed from sectionTitle
//     fontSize: 18,
//     fontWeight: '600',
//     color: '#333',
//     marginRight: 6,
